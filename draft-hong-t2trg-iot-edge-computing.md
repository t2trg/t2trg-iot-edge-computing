...
Low cost, massive availability of storage and processing power enabled the realization of a new computing model, in which virtualized resources can be leased in an on-demand fashion, being provided as general utilities.
...
Today, an unprecedented volume and variety of data is generated by Things and applications deployed in edge networks consume this data. 
...
A core function of IoT edge computing is to enable local computation on a node in the edge network, e.g. processing input data from sensors, making local decisions, reducing data, offloading computation on behalf of a device or user. Related functions include orchestrating computation (in a centralized or distributed manner) and managing applications lifecycle. Support for in-network computation may vary in term of capability, e.g., computing nodes can host virtual machines, software containers, software actors or unikernels able run stateful or stateless code, or a rule engine providing an API to register actions in response to conditions such as IoT device ID, sensor values to check, thresholds, etc.
